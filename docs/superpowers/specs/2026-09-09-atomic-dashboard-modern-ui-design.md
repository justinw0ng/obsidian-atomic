...
- `docs/images/atomic-dashboard.png` is captured by `npm run docs:user-guide-screenshots` (dashboard step in `scripts/capture-user-guide-screenshots.mjs`). The same run composes the README banner `atomic-dashboard-hero.png` from the desktop + 390×844 phone-width shots via `scripts/compose-device-hero.py`.
