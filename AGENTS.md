...
- To cut a GitHub Release from a Cloud Agent, follow the **Cursor Cloud Agents** subsection in `.cursor/skills/obsidian-plugin-e2e/references/release.md`. Users are prompted in-app with the latest update note (`en` → English body; `zh-Hant-en` / any `zh-Hant*` → Cantonese body) when `src/core/update-notes.json` `version` matches the installed plugin. Release workflow `release_notes` and `release_notes_zh_hant` inputs are **optional**. If both are set, the workflow writes that catalog (use `\n` for line breaks). If both are omitted, the catalog is left unchanged. Provide both languages or omit both. The default Cursor App token has `actions: read` only, so `gh workflow run` returns 403 unless a user-provided `GH_TOKEN` / `GITHUB_TOKEN` with Actions write is in Cloud Agent secrets.
