...
- To cut a GitHub Release from a Cloud Agent, follow the **Cursor Cloud Agents** subsection in `.cursor/skills/obsidian-plugin-e2e/references/release.md`. The default Cursor App token has `actions: read` only, so `gh workflow run` returns 403 unless a user-provided `GH_TOKEN` / `GITHUB_TOKEN` with Actions write is in Cloud Agent secrets.
