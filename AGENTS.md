...
- To cut a GitHub Release from a Cloud Agent, follow the **Cursor Cloud Agents** subsection in `.cursor/skills/obsidian-plugin-e2e/references/release.md`. When a release is confirmed, remind the owner to provide **English and Cantonese (zh-Hant)** update notes **or draft both covering all PRs since the last release** before or while cutting Release. Users are prompted in-app with the latest update note (`en` → English body; `zh-Hant-en` / any `zh-Hant*` → Cantonese body). Do not finalize a Release without both languages. Put them in `src/core/update-notes.json` (`body.en` and `body.zh-Hant`) and pass the same texts as the Release workflow `release_notes` and `release_notes_zh_hant` inputs (use `\n` for line breaks). The default Cursor App token has `actions: read` only, so `gh workflow run` returns 403 unless a user-provided `GH_TOKEN` / `GITHUB_TOKEN` with Actions write is in Cloud Agent secrets.
...
- After any UI or breaking change, update `e2e/health-check.test.mjs` and the `data-testid` hooks it uses (`atomic-heatmap`, `atomic-timer-*`, `atomic-gym-log-*`, `atomic-cues`, `atomic-bookshelf`, `atomic-book`, `atomic-setting-*`, `atomic-property-select`, `atomic-update-note-*`). Leaving Selenium coverage stale is a bug.
