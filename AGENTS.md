...
- To cut a GitHub Release from a Cloud Agent, follow the **Cursor Cloud Agents** subsection in `.cursor/skills/obsidian-plugin-e2e/references/release.md`. When a release is confirmed, remind the owner to provide an update note **or draft one covering all PRs since the last release** before or while cutting Release. Users are prompted in-app with the latest update note. Do not finalize a Release without that note. Put it in `src/core/update-notes.ts` (`UPDATE_NOTES`) and pass the same text as the Release workflow `release_notes` input. The default Cursor App token has `actions: read` only, so `gh workflow run` returns 403 unless a user-provided `GH_TOKEN` / `GITHUB_TOKEN` with Actions write is in Cloud Agent secrets.
