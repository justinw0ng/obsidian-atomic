...
- What's new update note after a version change (English and zh-Hant-en Notice; shows once; no nag)
...
Scripts: `npm run docs:user-guide-screenshots` (user-guide clips are looping GIFs), `scripts/capture-readme-hero.sh`, `npm run docs:hero-gif`, `npm run docs:cue-hero` (hover lift, then click to the centered larger card; `scripts/animate-cue-hero-gif.py`).
